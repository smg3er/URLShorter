...
Для первого локального запуска создайте `.env` из безопасного шаблона и заполните значения:

```bat
if not exist .env copy .env.example .env
```

`.env` исключён из Git. `.env.example` — только шаблон и во время работы не загружается.
